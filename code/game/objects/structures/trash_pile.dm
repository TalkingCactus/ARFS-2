...
/obj/structure/trash_pile/attack_hand(mob/user)
	...
	if(ishuman(user))
		...
		H.visible_message("[user] seraches through \the [src].","<span class='notice'>You search through \the [src].</span>")
		if(hider)
		...
		if(do_after(user,rand(4 SECONDS,6 SECONDS),src))
			...
			//You already searched this one bruh
				...
				to_chat(H,"<span class='warning'>There's not anything else for you to find in \the [src]!</span>")
			...
			//You found an item!
				...
				if(I)
					...
					to_chat(H,"<span class='notice'>You found [initial(I.name)]!</span>")
